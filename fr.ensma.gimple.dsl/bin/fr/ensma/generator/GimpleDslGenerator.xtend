...
//		var model = rs.allContents.head as TAC
	    val model = rs.resources.map(r|r.allContents.toIterable.filter(TAC)).flatten
		var content = model.compile
		var str = ""
...
//	    fsa.generateFile("capella.txt", model.compile)
	...
	def compile (Iterable<TAC> tacs) '''
	«FOR tac : tacs»
		«FOR functionDef : tac.functionDef»
			«'\t\t\t\t\t<ownedExtensions xsi:type="deployment:AADLFunction" id="654c1232-49f0-48dc-8f4e-fcb2ef42f336" name="'+functionDef.name.name+'">\n'»
			«FOR instruction : functionDef.codeBlock.instruction»
				«'\t\t\t\t\t\t<ownedExtensions xsi:type="deployment:AADLFunction" id="654c1232-49f0-48dc-8f4e-fcb2ef42f336" name="'+instruction.functionCall.calledFunction.name+'"/>\n'»
			«ENDFOR»
			« '\t\t\t\t\t</ownedExtensions>\n'»
		«ENDFOR»
	«ENDFOR»
